...
export default function ReturnDetailsReportPage() {
  ...
  const fetchReportData = async (signal) => {
    ...
    try {
      ...
      setTotals({
        returnQty: result?.returnQty ?? result?.ReturnQty ?? 0,
      ...
      if (err.name !== 'AbortError' && err.message !== 'canceled' && err.code !== 'ERR_CANCELED') {
        setError(err.message || 'Failed to fetch return details data.');
        setReportData([]);
      }
    } finally {
      setIsLoading(false);
  ...
  const handleNavigateToRecon = (row) => {
    let rowDate = '';
    if (row && (row.DATE || row.BILL_DATE)) {
      rowDate = String(row.DATE || row.BILL_DATE).split('T')[0];
    }
